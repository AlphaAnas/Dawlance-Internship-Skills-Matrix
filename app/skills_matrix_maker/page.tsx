...
// const CreateDepartmentDialog = ({ isOpen, onClose, onCreateDepartment, refreshDepartments }) => {
...
//         area: departmentArea.trim() || 'Production'
//       };
//       console.log(newDepartment);
//       // call the post api to save the new department
//       fetch('/api/post/departments', {
//         method: 'POST',
//         headers: {
//           'Content-Type': 'application/json'
//         },
//         body: JSON.stringify({ dept_name: newDepartment.name })
//       })
//       .then(response => response.json())
//       .then(data => {
//         if (data.success) {
//           // Refresh departments from API to get the latest data
//           refreshDepartments();
...
//   const [allEmployees, setAllEmployees] = useState<typeof initialEmployees>([]);
//   const [showCreateDepartment, setShowCreateDepartment] = useState(false);

//   // Fetch employees by department - more efficient than fetching all and filtering
...
//     try {
//       // Find department name from ID for API call
...
//       // Fetch employees filtered by department on the server side
//       const response = await fetch(`/api/all/employees?department=${encodeURIComponent(departmentName)}`);
...
//           department: departmentId, // Use the selected department ID
//           area: emp.area || department.area || 'Production',
//           joinDate: emp.join_date || new Date().toISOString().split('T')[0],
//           experience: emp.experience || '1 year'
//         }));
//         setAllEmployees(transformedEmployees);
//         console.log('Fetched employees for department:', departmentName, transformedEmployees);
//       } else {
...
//         if (data.success && data.data) {
//           // Transform API data to match the expected format
//           const transformedDepartments = data.data.map((dept: any) => ({
...
//         // Keep using initial departments as fallback
//       }
//     };

//     // Only fetch departments on mount - employees will be fetched when department is selected
//     fetchDepartments();
//   }, []);

//   // Effect to fetch employees when selected department changes
//   useEffect(() => {
...
//       setAllEmployees([]);
//     }
//   }, [selectedDepartment, departments]); // Include departments to ensure we have department data

//   // Function to refresh departments after creating a new one
...
//     if (selectedDepartment) {
//       await fetchEmployeesByDepartment(selectedDepartment);
//     }
//   };
...
//   // Since we're fetching employees by department directly, filtering is much simpler
...
//       setNewSkill('');
//     }
//     // insert the skills into the db
//     fetch('api/post/skills',
...
//                           console.log('All employees (for selected dept):', allEmployees);
//                           console.log('Selected department:', selectedDepartment);
//                           console.log('Filtered employees (by name):', filteredEmployees);
